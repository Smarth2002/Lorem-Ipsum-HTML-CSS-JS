...
form.addEventListener("submit", function (e){
    ...
    //to prevent it from submiting to server
    e.preventDefault();
    const value = parseInt(amount.value);
    
    //for these cases below display any random paragraph (0-9)
    //empty
    //<0
    //>9
    ...
    if(isNaN(value) || value <= 0 || value > 9){
        ...
        //display the paras from (0 - (value-1)) from text

        //ALTERNATIVE

        // let paras=[];
        // for(let i=0; i<value; i++){
        //     paras.push(`<p class="result">${text[i]}</p>`)
        // }
        // paras.join("");
